function [output] = vsm_no_pheno(T,P,phi,syear,eyear,tf_1,tf_2,tf_3,tf_4,wf_1,wf_2,wf_3,wf_4,SNo,Wo, ... ,
    rootd,rated,a_1,a_2,SNr,fdays,varargin)
...
%     while sum_temperature < Tg && tt < ndays(cyear) % as long as sum-of-temperatures has not crossed threshold to begin growth ...
...
%     
    ...
    for t = 1:ndays(cyear)  % begin cycling over days in a year, starting with the first day (fday) calculated above
        ...
        % start growth?
        if t >= fdays(cyear)     % only enter the growth and cambial blocks if it's time to start growth, otherwise, skip to end of the day cycle
...
output.parameters       = [tf_1,tf_2,tf_3,tf_4,wf_1,wf_2,wf_3,wf_4,SNo,Wo,Wmax,Wmin, ... ,
    rootd,rated,Pmax,k_1,k_2,k_3,Tm,a_1,a_2,SNr,SNmt];
