...
rootd = parameter(9);  % the root/soil melt depth (mm)

%rated = parameter(14);  % the rate of water drainage from soil
%Pmax = parameter(15);  % maximum rate of infiltration water into soil  (mm/day)

% interception and transpiration parameters
k_1 = parameter(10);  % k1 (1-k1) is the interception precipitation by the tree crown
k_2 = parameter(11);  % 1st coefficient for calculation the transpiration
k_3 = parameter(12);  % 2nd coefficient for calculation the transpiration

% soil and snow melting parameters
%Tm = parameter(19);  % sum of temperature for start soil melting (C)
%a_1 = parameter(20);  % 1st coefficient of soil melting
%a_2 = parameter(21);  % 2nd coefficient of soil melting
Tg = parameter(13);  % sum of temperature to start growth (C)
%SNr = parameter(23);  % the rate of snow melting (mm/C/day)
%SNmt = parameter(24);  % minimum temperature for snow melting

% Some switches and variable storage 
% K_8 = parameter(25);  % Maximum duration (days) of latewood formation
%K_9 = parameter(25);  % the period (days) over which to sum temperature to calculate start of growth
...
output = vsm_me(T,P,phi,syear,eyear,tf_1,tf_2,tf_3,tf_4,wf_1,wf_2,wf_3,wf_4,rootd,k_1,k_2,k_3,Tg);
toc %  
